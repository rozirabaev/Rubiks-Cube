...
{
	scn = scene;
	MoveCamera(0, zTranslate, 15);
	glm::ivec4 viewport;
	glGetIntegerv(GL_VIEWPORT, &viewport[0]);
	drawInfo.push_back(new DrawInfo(0, 0, 0, 0,   inAction |depthTest| toClear | blackClear));
	buffers.push_back(new DrawBuffer());

	if (xViewport.empty() && yViewport.empty())
	{
		viewports.push_back(viewport);
		drawInfo.push_back(new DrawInfo(0, 0, 1, 0,  toClear | depthTest |  blackClear));
	}
	else
	{
...
bool Renderer::Picking(float x, float y)
{
	...
	glReadPixels((int)x, viewport[3] - (int)y, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, data);
	glReadPixels((int)x, viewport[3] - (int)y, 1, 1, GL_RGBA, GL_FLOAT, data_f);
	glReadPixels((int)x, viewport[3] - (int)y, 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &depth);
	

	return scn->Picking(data,x,y);
//	return false;
...
void Renderer::rotate_wall(int index) {
	//scn->rotate_wall(index);
